...
export const signup = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDyVoB79T0hXqXTVENd-Cb_ilZP_HvVWyc",
      {
        method: "POST",
        headers: {
...
export const login = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDyVoB79T0hXqXTVENd-Cb_ilZP_HvVWyc",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
